...
async def get_price(address: str):
    # async with aiohttp.ClientSession(headers=env.HEADERS) as session:
    #     async with session.get(f'{env.API_URL}/price?address={address}') as response:
...
# todo: error handling
async def get_multi_price(addresses: list[str]):
    # async with aiohttp.ClientSession(headers=HEADERS) as session:
    #     async with session.get(f'{env.API_URL}/multi_price?list_address={",".join(addresses)}') as response:
    #         return await response.json()
    return {
        "data": {
